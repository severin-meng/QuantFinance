...
def ad_black_scholes(S, t, K, T, r, vol):
    ...
    t_bar += y_2_bar * -vol / (2 * sqrt(T-t))

    # 1. y_1 = log(S/K) + (r + vol^2/2) * (T-t)
    S_bar += y_1_bar * 1/S
    r_bar += y_1_bar * (T-t)
    vol_bar += y_1_bar * vol * (T-t)
    t_bar += y_1_bar * (r + vol**2 / 2) * (-1)
...
if __name__ == "__main__":
    ...
    import pandas as pd
    data = []
    data.append(["Analytisch"] + list(black_scholes_analytical(S, t, K, T, r, vol)))
    data.append(["Adjoint"] + list(ad_black_scholes(S, t, K, T, r, vol)))
    data.append(["Bump&Reprice"] + list(bump_reprice_black_scholes(S, t, K, T, r, vol)))
